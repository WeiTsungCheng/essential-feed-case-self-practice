...
final class URLSessionHTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_succeedWithEmptyDataHTTPURLResponseWithNilData() {
        
        let response = anyHTTPURLResponse()
        let receivedValues = resultValuedFor((data: nil, response: response, error: nil))
        let emptyData = Data()
    ...
    private func resultValuedFor(_ values: (data: Data?, response: URLResponse?, error: Error?), file: StaticString = #filePath, line: UInt = #line) -> (data: Data, response: HTTPURLResponse)? {
